...
// Get user's servers (authenticated - returns current user's servers)
router.get('/user/:userId', (req, res) => {
  ...
  if (parseInt(req.params.userId) !== userId) {
    return res.status(403).json({ error: 'Cannot view other users servers' });
  }
...
// Get server messages (from first channel) - requires membership
router.get('/:id/messages', (req, res) => {
  ...
  db.get('SELECT * FROM server_members WHERE server_id = ? AND user_id = ?', 
    [serverId, userId], (err, membership) => {
      ...
      db.get('SELECT id FROM channels WHERE server_id = ? ORDER BY id ASC LIMIT 1', [serverId], (err, channel) => {
        if (!channel) {
          return res.json([]);
        }
...
router.get('/pending-setup/:userId', (req, res) => {
  ...
  // Only allow users to see their own pending setups
  if (parseInt(req.params.userId) !== userId) {
    return res.status(403).json({ error: 'Cannot view other users pending setups' });
  }
